  ...
  <div class="min-h-screen flex overflow-x-hidden" style="background: #f8faf9;">
    ...
    >
      ...
      <nav class="flex-1 px-2 py-2 space-y-0.5 overflow-y-auto">
        ...
        <SidebarLink :to="`/partner/${orgId}/locations`" :collapsed="!sidebarOpen" icon="pin" label="Localizações" />
        <SidebarLink v-if="canAdmin" :to="`/partner/${orgId}/members`" :collapsed="!sidebarOpen" icon="users" label="Membros" />
        <SidebarLink v-if="canAdmin" :to="`/partner/${orgId}/audit`" :collapsed="!sidebarOpen" icon="audit" label="Auditoria" />
      </nav>
...
const icons = {
  ...
  audit: 'M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01'
}
